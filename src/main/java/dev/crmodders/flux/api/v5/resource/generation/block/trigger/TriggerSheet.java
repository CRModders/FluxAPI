package dev.crmodders.flux.api.v5.resource.generation.block.trigger;

import dev.crmodders.flux.api.v5.resource.ResourceLocation;
import dev.crmodders.flux.api.v5.resource.generation.Writeable;
import dev.crmodders.flux.api.v5.resource.generation.block.BlockAction;
...
public class TriggerSheet extends Writeable {
     ...
     */
    public void removeActionByComparison(ReturnableInputSupplier<BlockAction, Boolean> comparator) {
     ...
     * @param {BlockActionComparator} comparator The function to use to select actions.
     */
    public void removeTriggerActionByComparison(String trigger, ReturnableInputSupplier<BlockAction, Boolean> comparator) {
        ...
        for(BlockAction action : actions) {
            if(comparator.get(action)) deletions.add(action);
        }
